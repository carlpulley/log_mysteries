namespace :add do
  namespace :page do
    task :resources => :environment do
      events = LogEvent.tagged_with("wordpress").ip_address("10.0.1.2").user_agent("Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.6; en-US; rv:1.9.2.3) Gecko/20100401 Firefox/3.6.3")
      ...
      events = LogEvent.tagged_with("wordpress").ip_address("10.0.1.2").user_agent("Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_2; en-us) AppleWebKit/531.22.7 (KHTML, like Gecko) Version/4.0.5 Safari/531.22.7")
      ...
      events = LogEvent.tagged_with("wordpress").ip_address("190.166.87.164")
      ...
      events = LogEvent.tagged_with("wordpress").ip_address("190.167.70.87")
      events.where(:name => "www-media.log").url("/wp-content").all.each do |event|
      ...
      events = LogEvent.tagged_with("wordpress").ip_address("65.88.2.5")

class Sudo < Auth
  ...
  def message_to_s
    ...
    return "#{self.message[:subject] == "user1" ? ' '*2 : ' '*4}#{self.message[:subject]} : #{self.message[:error]} ; TTY=#{self.message[:tty]} ; PWD=#{self.message[:pwd]} ; USER=#{self.message[:user]} ; COMMAND=#{self.message[:command]}" if self.message.keys.count == 6
